...
- **시크릿 관리 방식 미확정** ([ADR-0007](../02-adr/0007-secret-management.md) Proposed). 시크릿 저장소는 추상 박스로 표기.
...
C4Container
    ...
    System_Boundary(cp, "셀프호스팅 클라우드 플랫폼 (컨트롤 플레인) — 단일 Go 바이너리") {
        ...
        Container(secrets, "시크릿 저장소", "미확정 (ADR-0007)", "Proxmox API Token, DB 자격증명, JWT 서명 키, OIDC client secret, PG 어댑터 자격증명 등 전 시크릿. 평문 노출 금지")
    }
...
| 시크릿 저장소 | Proxmox API Token·DB 자격증명·JWT 서명 키·OIDC client secret·PG 자격증명 등 **전 시크릿**(TB-7 소비자: 어댑터·API·빌링). 평문 노출 금지 | vision §3.2, ADR-0007 |
